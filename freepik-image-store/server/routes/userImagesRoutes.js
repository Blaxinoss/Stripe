    ...
    try {
      ...
      if (image.downloadCount >= image.maxDownloads) {
        return res
          ...
          .json({
            message:
              "Maximum download limit already reached how did you get here",
          });
      }
...
router.put(
  "/user-images/:imageId/update-download-url",
  passport.authenticate("jwt", { session: false }),
  async (req, res) => {
    try {
      const userId = req.user._id;
      const { imageId } = req.params;
      const { newDownloadUrl } = req.body;

      if (!newDownloadUrl) {
        return res.status(400).json({ message: "Missing new download URL" });
      }
      ...
      image.downloadUrl = newDownloadUrl;
      await image.save();
